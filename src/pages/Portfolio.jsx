	...
	return (
		<div className="portfolio fade-up">
			...
			<div style={{
				...
				background: 'rgba(26,22,37,0.8)', padding: '28px 32px 32px'
			}}>
				<Skeleton width="120px" height="22px" radius="100px" style={{ marginBottom: 24 }} />
				<div style={{ display: 'flex', gap: 28 }}>
					<Skeleton width="100px" height="100px" radius="16px" style={{ flexShrink: 0 }} />
					<div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
						<Skeleton width="180px" height="26px" radius="6px" />
						<Skeleton width="100px" height="14px" radius="4px" />
						<Skeleton width="100%" height="13px" radius="4px" />
						<Skeleton width="85%" height="13px" radius="4px" />
					</div>
				</div>
			</div>
			{/* Grid skeleton */}
			<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
				{[0, 1].map(i => (
					<div key={i} style={{
						...
						background: 'rgba(26,22,37,0.8)', padding: '22px 24px'
					}}>
						<Skeleton width="80px" height="14px" radius="5px" style={{ marginBottom: 18 }} />
						{[0, 1, 2, 3].map(j => (
							<div key={j} style={{ marginBottom: 12 }}>
								<Skeleton width="100%" height="12px" radius="4px" style={{ marginBottom: 6 }} />
...
export default function Portfolio() {
	...
	useEffect(() => {
		async function fetchAll() {
			try {
				const [profileRes, skillsRes, achRes, projRes] = await Promise.all([
					fetch(`${API}/profile`),
					fetch(`${API}/skills`),
				...
				const [profileData, skillsData, achData, projData] = await Promise.all([
					profileRes.json(),
					skillsRes.ok ? skillsRes.json() : null,
					achRes.ok ? achRes.json() : null,
	...
	// skills and achievements may be a document with an array field
	const skillsList = Array.isArray(skills) ? skills : (skills?.items || skills?.skills || []);
	const achievementsList = Array.isArray(achievements) ? achievements : (achievements?.items || achievements?.achievements || []);
										...
										<div className="project-meta">
											{p.lang && (
												<span className="project-lang">
													<span className="lang-dot" style={{ background: LANG_COLORS[p.lang] || '#888' }} />
													{p.lang}
												</span>
											)}
											{p.stars !== undefined && (
